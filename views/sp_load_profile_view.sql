CREATE OR REPLACE VIEW DM_DATASCIENCE.SP_LOAD_PROFILE_VIEW AS
SELECT 
AM.SPID AS SPID,
AM.METER_NUMBER AS METERID,
AM.SERVICE_TYPE AS SPTYPE,
CP.PREM_TYPE_CD AS PREMISETYPE,
COALESCE(EV_SP.EV_COUNT,0) AS SP_EV_COUNT,
COALESCE(AG_SP.SYSTEM_CAPACITY_KW,0) AS SP_PVSIZE,
...
FROM 
WAREHOUSE.HUB_GIS_ASSET_MAP AM
LEFT JOIN (SELECT TRANSFOMERID, CUSTOMER_COUNT
      FROM WAREHOUSE.TRANS_CUSTCOUNT
     ) CCT
ON CCT.TRANSFOMERID = AM.BANK_OBJECTID
LEFT JOIN WAREHOUSE.HUB_CUST_SERVICE_POINT SP
ON AM.SPID = SP.SP_ID 
LEFT JOIN WAREHOUSE.HUB_CUST_PREMISE CP
ON SP.PREMISE_ID = CP.PREMISE_ID
...
LEFT JOIN (SELECT SPID,COUNT(DISTINCT(VIN_REDUCED)) AS EV_COUNT  FROM EV.CLEAN_EV_ASSET_MAP GROUP BY SPID ) EV_SP
ON AM.SPID = EV_SP.SPID
...
LEFT JOIN (SELECT SPID,SUM(SYSTEM_CAPACITY_KW) AS SYSTEM_CAPACITY_KW FROM NEM.SP_ACTIVEGENERATORS 
			WHERE UPPER(TYPE_OF_GENERATOR) IN ('S', 'S ', 'SOLAR', 'SOLAR ') AND SPID !=''
			GROUP BY SPID)AG_SP
ON AM.SPID = AG_SP.SPID
