CREATE OR REPLACE TASK DM_DATASCIENCE.task_olt_calendardates_data
WAREHOUSE = DATA_INGESTION_OLT,
SCHEDULE ='USING CRON 30 2 * * * America/Los_Angeles'
AS
COPY INTO @public.dsm_api_olt/incremental/overloaded_transformer_calendardates/overloaded_transformer_calendardates
FROM (	select * from(
        Select
...
ALTER TASK DM_DATASCIENCE.task_olt_calendardates_data resume;